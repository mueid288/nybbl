...
import { getJobs, getAssignments, getMembers, getTimelogs, getUpdates } from '../../lib/store.js';
    ...
    async run() {
        ...
        for (const job of activeJobs) {
            ...
            for (const member of teamMembers) {
                const memberUpdates = todayUpdates.filter(
                    u => u.member === member!.handle && u.job === job.id
                );
