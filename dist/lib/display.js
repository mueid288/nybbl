...
export function printLogo(version = '1.0.0') {
    console.log('');
...
export function printBox(title, message) {
...
export function printHeader(emoji, text) {
    console.log('');
...
export function formatStreak(streak) {
    if (streak === 0)
        return chalk.hex('#555')('○ 0');
    if (streak <= 3)
        return chalk.hex('#888')(`🔥 ${streak}`);
    if (streak <= 7)
        return chalk.hex('#f4a261')(`🔥 ${streak}`);
    return chalk.hex('#e63946').bold(`🔥 ${streak}`);
}
// ─── Welcome Message (time-based) ────────
export function getWelcomeMessage(handle) {
...
// ─── Live Timer Elapsed ──────────────────
export function formatElapsedTimer(startTime) {
...
export function getMemberColor(handle) {
    let hash = 0;
    for (const ch of handle)
        hash = ((hash << 5) - hash + ch.charCodeAt(0)) | 0;
    return MEMBER_COLORS[Math.abs(hash) % MEMBER_COLORS.length];
}
export function colorMember(handle, isMe = false) {
...
// ─── Status Bar ─────────────────────────
export function printStatusBar(parts) {
    const joined = parts.join(chalk.hex('#555')(' · '));
    console.log(chalk.hex('#555')('  │ ') + joined + chalk.hex('#555')(' │'));
    console.log('');
}
// ─── Section Box ───────────────────────
export function printSectionBox(title, content, borderColor = '#06d6a0') {
