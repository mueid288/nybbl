...
export function getLockedAchievements(data) {
    return ACHIEVEMENTS.filter(a => !a.check(data));
}
export function formatAchievementBadges(unlocked) {
    if (unlocked.length === 0)
        return chalk.gray('No badges yet — keep going!');
    return unlocked.map(a => a.emoji).join(' ');
}
export function formatAchievementList(unlocked, locked) {
    const lines = [];
    if (unlocked.length > 0) {
